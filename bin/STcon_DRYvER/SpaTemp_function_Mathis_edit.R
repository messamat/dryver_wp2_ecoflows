...
spat_temp_index <- function(interm_dataset, 
  ...
  ################################### NOT NEEDED FOR THE REST OF THE CODE#######
  ...
  #   
  #   ST_matrix_out <- matrix(nrow = ncol(interm_dataset[[river]])-1,
  #                           ncol = ncol(interm_dataset[[river]])-1, 
  ...
  #                     which(Network_stru[[river]][site_step,]==1)] <- 1
  #     }else{
  #       ST_matrix_out[spa_connections[site_step],
  #                     which(Network_stru[[river]][site_step,]==1)] <- 0
  #     }
  #   }
  ...
  pack_check_val <- length(which(pack_check=="package:sna"))
  if(pack_check_val>0){detach("package:sna", unload = TRUE)}
  out_Matrix_LIST <- foreach(river=1:length(interm_dataset))%dopar%{
  ...
  # We calculate the number of nodes of our network (used along the function)  
    interm_ncols <- ncol(interm_dataset[[river]])
    numn_nodes <- interm_ncols - 1
    nsteps <- length(interm_dataset[[river]][,1])
    ...
    #Create matrices of n_time_steps rows x n_sites cols
    ST_Oclosenness_matrix <- matrix(nsteps, numn_nodes, data = value_NO_S_link)
    ST_Allclosenness_matrix <- matrix(nsteps, numn_nodes, data = value_NO_S_link)
    ST_betweennes_matrix <- matrix(nsteps, numn_nodes, data = value_NO_S_link)
    ...
    for (days in 1:(nsteps-1)) {
      cat("We are at time unit", days, "of", (nsteps-1), "and at river", river,"\n")
      ...
      time_step_1 <- interm_dataset[[river]][days, 2:interm_ncols]
      time_step_2 <- interm_dataset[[river]][days+1, 2:interm_ncols]
      if(weighting_links==T){day_link_weights <- link_weights[[river]][days,2:interm_ncols]}
      ...
      # for (site_step in spa_connections) {
      #   if(time_step_1[site_step]==1){
      #     ST_matrix_netwGraph[site_step, spa_connections] <-
      #       as.numeric(Network_stru[[river]][site_step,])
      #   }else{
      #     ST_matrix_netwGraph[site_step, spa_connections[-site_step]] <- 0
      #   }
      # }
      #####Mathis simplified interpretation of the above lines:
      ...
      ST_matrix_netwGraph[time_step_1==1,] <- 
        as.matrix(Network_stru[[river]])[time_step_1==1,]
      ...
      #   check <- length(all_shortest_paths(a, every_path, 1:numn_nodes, mode = sense)$res) #######REMARK Mathis: here, I don't understand why it includes the path to itself here, it seems that check will always be > 0
      #   if (check==0) {
      #     site <-0
      #   }else{
      ...
      #   temp_connections <- seq(1 + numn_nodes, 2*numn_nodes)#+((days)*numn_nodes) 
      #   
      ...
      #           All_river_paths_legacy+ST_matrix[spa_connections[site_step],
      #                                            temp_connections[1]:temp_connections[numn_nodes]]
      #         
      ...
      #         if(weighting_links==T){
      #           value_T_LINK_modif <- 
      #             value_T_LINK_modif*as.numeric(day_link_weights[site_step])
      #         }
      #         
      #         ST_matrix[spa_connections[site_step],
      #                   temp_connections[site_step]] <- 
      #           (value_T_LINK_modif*legacy_effect[leg_eff]) + 
      #           ST_matrix[spa_connections[site_step],temp_connections[site_step]]
      #       }
      #     } else {# Here we check if the temporal change implies going from 0 to 0 (so a stable disconnected link). Then we put 0
      #       
      #       # We weight the links base on daily information of flow or strength of the link.
      #       if(weighting_links==T){
      #         All_river_paths[site_step,] <-  
      ...
      #   }
      #   
      #   ##############MATHIS Remark: I do not understand this. It is under the assumption that everytime
      #   ############## a reach dries after having been wet, there is a peak in connectivity beforehand?
      #   ############## in this case, I don't really see why we add the legacy on top of this
      ...
      #       as.numeric(All_river_paths[site_step,]*
      #                    as.numeric(day_link_weights[site_step]))}
      #     # We weight
      #     if(weighting==T){
      #       All_river_paths[site_step,] <- as.numeric(
      #         All_river_paths[site_step,]*dist_matr[site_step,])}
      #     
      ...
      # 8. Apply legacy effects and update ST_matrix
      ...
      ST_matrix[spa_connections[stable_indices_1], temp_connections[stable_indices_1]] <- 
        ST_matrix[spa_connections[stable_indices_1], temp_connections[stable_indices_1]] + value_T_LINK_modif
