...
DRN_position=1 # For looping between catchments 
...
cat_to_correct <- shape_DRN %>% arrange(from) %>% group_by(cat) %>% mutate(count=n()) %>%  filter(count>1)%>% pull(cat)
if (length(cat_to_correct)>0) {
  shape_DRN[which(shape_DRN$cat%in%cat_to_correct),4]$cat <- paste(cat_to_correct,rep(LETTERS[1:length(which(cat_to_correct-cat_to_correct[1]==0))]),sep="_")
}
#___________________________________________________________________

# We built the adjacency list to built the graph
DRN_adj_table <- shape_DRN  %>% arrange(from) %>% select(from,to,to_cat_shp) %>% sf::st_drop_geometry()
...
End_point <- flow_intermittence %>% filter(reachID==flow_intermittence$reachID[1]) %>%# We select whatever reach
                                    rename("cat"="reachID") %>% 
