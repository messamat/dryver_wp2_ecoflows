# DRYvER 

# Site-specific  assembly processes
# - How does site-specific dying affect local assembly processes? (cf. Huttunen et al. 2017)


rm(list = ls())
library(data.table)
library(vegan)

# FUNGI - biofilm
# ------------

fungi_biof <- fread("M:/gDRYvER/WP 2/Metacommunity models/Datasetit/fungi_dna_Biof_removed_zero_rows_and_columns.csv")
fungi_biof[1:10, 1:10]

levels(factor(fungi_biof$Country))

#### Croatia ####
com <- fungi_biof[Country == "Croatia"] 

tot <- rowSums(com[,6:1982])
com <- com[tot > 0, ]

tt <- table(com$Site)
rare_levels <- names(tt)[tt<2]
com <- subset(com,!Site %in% rare_levels)

groups <- as.factor(com$Site)
com[1:10, 1:10]
com <- com[,6:1982]

### The models start here
start_time <- Sys.time()
foo <-function(x, groups, ...) diag(meandist(vegdist(x, "jac", binary =TRUE), grouping = groups))
output_Croatia <- oecosimu(com, foo, "quasiswap", nsimul = 999, groups = groups)
end_time <- Sys.time()
end_time - start_time
# Saving
sink("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/CRO_biofilm_fungi_null_models_999perm.txt")
paste("-----")
paste("jaccard, null model")
output_Croatia
sink()

#### Czech ####
com <- fungi_biof[Country == "Czech"] 

tot <- rowSums(com[,6:1982])
com <- com[tot > 0, ]

# tt <- table(com$Site)
# rare_levels <- names(tt)[tt<2]
# com <- subset(com,!Site %in% rare_levels)

groups <- as.factor(com$Site)
com[1:10, 1:10]
com <- com[,6:1982]

### The models start here
start_time <- Sys.time()
foo <-function(x, groups, ...) diag(meandist(vegdist(x, "jac", binary =TRUE), grouping = groups))
output_Czech <- oecosimu(com, foo, "quasiswap", nsimul = 999, groups = groups)
end_time <- Sys.time()
end_time - start_time
# Saving
sink("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/CZE_biofilm_fungi_null_models_999perm.txt")
paste("-----")
paste("jaccard, null model")
output_Czech
sink()


#### Finland ####
com <- fungi_biof[Country == "Finland"] 

tot <- rowSums(com[,6:1982])
com <- com[tot > 0, ]

# tt <- table(com$Site)
# rare_levels <- names(tt)[tt<2]
# com <- subset(com,!Site %in% rare_levels)

groups <- as.factor(com$Site)
com[1:10, 1:10]
com <- com[,6:1982]

### The models start here
start_time <- Sys.time()
foo <-function(x, groups, ...) diag(meandist(vegdist(x, "jac", binary =TRUE), grouping = groups))
output_Finland <- oecosimu(com, foo, "quasiswap", nsimul = 999, groups = groups)
end_time <- Sys.time()
end_time - start_time
# Saving
sink("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/FIN_biofilm_fungi_null_models_999perm.txt")
paste("-----")
paste("jaccard, null model")
output_Finland
sink()


#### France ####
com <- fungi_biof[Country == "France"] 

tot <- rowSums(com[,6:1982])
com <- com[tot > 0, ]

# tt <- table(com$Site)
# rare_levels <- names(tt)[tt<2]
# com <- subset(com,!Site %in% rare_levels)

groups <- as.factor(com$Site)
com[1:10, 1:10]
com <- com[,6:1982]

### The models start here
start_time <- Sys.time()
foo <-function(x, groups, ...) diag(meandist(vegdist(x, "jac", binary =TRUE), grouping = groups))
output_France <- oecosimu(com, foo, "quasiswap", nsimul = 999, groups = groups)
end_time <- Sys.time()
end_time - start_time
# Saving
sink("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/FRA_biofilm_fungi_null_models_999perm.txt")
paste("-----")
paste("jaccard, null model")
output_France
sink()


#### Hungary ####
com <- fungi_biof[Country == "Hungary"] 

tot <- rowSums(com[,6:1982])
com <- com[tot > 0, ]

# tt <- table(com$Site)
# rare_levels <- names(tt)[tt<2]
# com <- subset(com,!Site %in% rare_levels)

groups <- as.factor(com$Site)
com[1:10, 1:10]
com <- com[,6:1982]

### The models start here
start_time <- Sys.time()
foo <-function(x, groups, ...) diag(meandist(vegdist(x, "jac", binary =TRUE), grouping = groups))
output_Hungary <- oecosimu(com, foo, "quasiswap", nsimul = 999, groups = groups)
end_time <- Sys.time()
end_time - start_time
# Saving
sink("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/HUN_biofilm_fungi_null_models_999perm.txt")
paste("-----")
paste("jaccard, null model")
output_Hungary
sink()


#### Spain ####
com <- fungi_biof[Country == "Spain"] 

tot <- rowSums(com[,6:1982])
com <- com[tot > 0, ]

tt <- table(com$Site)
rare_levels <- names(tt)[tt<2]
com <- subset(com,!Site %in% rare_levels)

groups <- as.factor(com$Site)
com[1:10, 1:10]
com <- com[,6:1982]

### The models start here
start_time <- Sys.time()
foo <-function(x, groups, ...) diag(meandist(vegdist(x, "jac", binary =TRUE), grouping = groups))
output_Spain <- oecosimu(com, foo, "quasiswap", nsimul = 999, groups = groups)
end_time <- Sys.time()
end_time - start_time
# Saving
sink("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/SPA_biofilm_fungi_null_models_999perm.txt")
paste("-----")
paste("jaccard, null model")
output_Spain
sink()

#### next ####

# Next : save all outputs to same dataframe !! 

res1 <- cbind(as.data.frame(output_Croatia$oecosimu$statistic), as.data.frame(output_Croatia$oecosimu$z), as.data.frame(output_Croatia$oecosimu$pval))
colnames(res1) <- c('Statistic','SES','P') 
res1$Country <- "Croatia"

res2 <- cbind(as.data.frame(output_Czech$oecosimu$statistic), as.data.frame(output_Czech$oecosimu$z), as.data.frame(output_Czech$oecosimu$pval))
colnames(res2) <- c('Statistic','SES','P') 
res2$Country <- "Czech Republic"

res3 <- cbind(as.data.frame(output_Finland$oecosimu$statistic), as.data.frame(output_Finland$oecosimu$z), as.data.frame(output_Finland$oecosimu$pval))
colnames(res3) <- c('Statistic','SES','P') 
res3$Country <- "Finland"

res4 <- cbind(as.data.frame(output_France$oecosimu$statistic), as.data.frame(output_France$oecosimu$z), as.data.frame(output_France$oecosimu$pval))
colnames(res4) <- c('Statistic','SES','P') 
res4$Country <- "France"

res5 <- cbind(as.data.frame(output_Hungary$oecosimu$statistic), as.data.frame(output_Hungary$oecosimu$z), as.data.frame(output_Hungary$oecosimu$pval))
colnames(res5) <- c('Statistic','SES','P') 
res5$Country <- "Hungary"

res6 <- cbind(as.data.frame(output_Spain$oecosimu$statistic), as.data.frame(output_Spain$oecosimu$z), as.data.frame(output_Spain$oecosimu$pval))
colnames(res6) <- c('Statistic','SES','P') 
res6$Country <- "Spain"

res_biof_fungi <- rbind(res1, res2, res3, res4, res5, res6)
res_biof_fungi$Organism <- "Biofilm_fungi"

save(res_biof_fungi, file = "M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/Biofilm_fungi_jaccard_oecosimu_999p.Rdata")
# Next : save R workspace
save.image(file = "M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/Biofilm_fungi_jaccard_oecosimu_999p_workspace.Rdata")

# load("M:/gDRYvER/WP 2/Metacommunity models/Tulokset/Null models/Biofilm_fungi_jaccard_oecosimu_999p.Rdata")
