...
SES_TotDur90_fun_sedi <- all[organism=='fun_sedi',
                             cor.test(z, TotDur90),
                             by=Country] 
SES_discharge_fun_sedi <- all[organism=='fun_sedi',
                             cor.test(z, discharge),
                             by=Country] 
...
for(i in unique(all$Country)){
  ...
  plots[[paste0(i)]] <- ggplot(d[organism=="miv"], aes(x=z, y=Site, color=stream_type)) + 
...
pdf(file.path(resdir, "Null_models", "Macroinvertebrates_SES_vs_streamtype.pdf"), 
    height=10, width=10)
...
p1
pdf(file.path(resdir, "Null_models", "All_Sediment_diatoms_SES_vs_TotDur90_lm2.pdf"), height=3, width=4) # MUOKKAA
p1
dev.off()

# TotLeng90
summary(lm(ss$z~ ss$TotLeng90))
...
pdf(file.path(resdir, "Null_models", "All_Sediment_diatoms_SES_vs_TotLeng90_lm2.pdf"), 
    height=3, width=4) 
p1
dev.off()
...
pdf(file.path(resdir, "Null_models", "All_Sediment_diatoms_SES_vs_Discharge_lm2.pdf"), 
    height=3, width=4) 
p1
dev.off()
...
lmer_bac_sedi <- all[
  ...
  list(
    TotDur90_full = lmer(z ~ TotDur90 + (1|Country), data=.SD),
    TotDur90_null = lmer(z ~ (1|Country), data=.SD),
    TotDur90_ML = list(anova(lmer(z ~ TotDur90 + (1|Country), data=.SD),
                        lmer(z ~ (1|Country), data=.SD))),
    TotLeng90_full = lmer(z ~ TotLeng90 + (1|Country), data=.SD),
    TotLeng90_null = lmer(z ~ (1|Country), data=.SD),
...
ss <- all[organism == "bac_biof_nopools"] 
jitter1 <- ggplot(ss) + 
  scale_y_continuous() +
  geom_jitter(aes(Country, z, colour=significance), shape=16, size = 2, alpha=0.7, position=position_jitter(0.2)) +
  theme_classic() +
...
ss <- all[organism == "bac_sedi_nopools"] 
jitter2 <- ggplot(ss) + 
  scale_y_continuous() +
  geom_jitter(aes(Country, z, colour=significance), shape=16, size = 2, alpha=0.7, position=position_jitter(0.2)) +
  theme_classic() +
...
levels(factor(all$all))

ss <- all[organism == "fun_biof"] 
jitter5 <- ggplot(ss) + 
  scale_y_continuous() +
  geom_jitter(aes(Country, z, colour=significance), shape=16, size = 2, alpha=0.7, position=position_jitter(0.2)) +
  theme_classic() +
